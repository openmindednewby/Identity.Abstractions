# Building and Deploying Identity.Abstractions
...
```bash
git clone https://github.com/openmindednewby/Identity.Abstractions.git
cd Identity.Abstractions
```
...
dotnet add package Identity.Abstractions --version 1.0.0 --source LocalFeed
```
...
```xml
<ItemGroup>
  <PackageReference Include="Identity.Abstractions" Version="1.0.0">
    <Source>C:\path\to\Identity.Abstractions\artifacts\Identity.Abstractions.1.0.0.nupkg</Source>
  </PackageReference>
</ItemGroup>
```
...
2. Click **Create**
3. Provide a key name (e.g., "Identity.Abstractions")
4. Select **Push** scope
5. Select packages or glob pattern (e.g., `Identity.Abstractions*`)
6. Copy the generated API key

### Step 2: Publish

```bash
dotnet nuget push ./artifacts/Identity.Abstractions.1.0.0.nupkg \
...
https://www.nuget.org/packages/Identity.Abstractions
...
unzip Identity.Abstractions.1.0.0.nupkg -d extracted/
...
```
error: Response status code does not indicate success: 409 (Conflict - The feed already contains 'Identity.Abstractions' version '1.0.0'.)
```
...
- 📦 **NuGet**: [NuGet Package Page](https://www.nuget.org/packages/Identity.Abstractions)
